...
func NewXPUBind(clientset *kubernetes.Clientset, c *cache.SchedulerCache) *Bind {
	return &Bind{
		Name: "xpusharesbinding",
		Func: func(name string, namespace string, podUID types.UID, node string, c *cache.SchedulerCache) error {
			...
			if err != nil {
				log.Printf("warn: failed to handle pod %s in namespace %s due to error %v", name, namespace, err)
				return err
			}
...
func getPod(name string, namespace string, podUID types.UID, clientset *kubernetes.Clientset, c *cache.SchedulerCache) (pod *v1.Pod, err error) {
	...
	if pod.UID != podUID {
		...
		if pod.UID != podUID {
			return nil, fmt.Errorf("the pod %s in namespace %s's uid is %v, and it's not equal with expected %v",
				name,
				namespace,
				pod.UID,
				podUID)
